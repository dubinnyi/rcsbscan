...
def get_coordinates(filename, fmt, other_args=None):
    """
    ...
    if fmt == "xyz" or fmt == "xyzgz" or fmt == "xyz.gz":
        ...
        eprint("Could not recognize file format: {:s}".format(fmt))
        quit()

    return get_func(filename, other_args)


def get_coordinates_pdb(filename, chain=None, res_range=None, atom_names=None):
    """
    ...
    # print("Specian parameters for {}:".format(filename))
    # if chain:
    #     print("chain={}".format(chain))
    # if res_range:
    #     print("res_range={}".format(res_range))
    # if atom_names:
    ...
    #
    # ATOM     42 HD21 ASN A 375       6.749  -0.333 339.015  1.00  0.00           H
    #
    ...
    with openfunc(filename, openarg) as f:
        ...
        for line in lines:
            ...
            if line.startswith("ATOM"):
                ...
                #  Selection by chain, res_range, atom_type
                selection_flag = True
                if chain or res_range or atom_names:
                    try:
                        atom_name = line[11:16].strip()
                        res_name = line[16:21].strip()
                        chain_name = line[21].strip()
                        res_number = int(line[22:26].strip())
                        # print("res_number = {}, atom_name = {}, chain_name = {}".format(res_number, atom_name, chain_name))
                        if (chain and not chain == chain_name) \
                                or (res_range and not (res_number in res_range)) \
                                or (atom_names and not (atom_name in atom_names)):
                            selection_flag = False
                    except:
                        msg = "error: Parsing of residue, chain and/or atomtype " + \
                              "for the following line:\n{0:s}".format(line)
                        eprint(msg)
                        quit()
                if not selection_flag:
                    # print("SKIP:{}".format(line.strip()))
                    continue
                ...
                try:
                    ...
                    msg = "error: Parsing atomtype for the following line: \n{0:s}".format(line)
                    eprint(msg)
                    quit()
                ...
                if x_column == None:
                    try:
                        ...
                        msg = "error: Parsing coordinates for the following line: \n{0:s}".format(line)
                        eprint(msg)
                        quit()
                ...
                try:
                    ...
                    try:
                        ...
                        msg = "error: Parsing input for the following line: \n{0:s}".format(line)
                        eprint(msg)
                        quit()
...
"""
    ...
    # Rotation
    parser.add_argument('-r', '--rotation', action='store', default="kabsch",
                        help='select rotation method. "kabsch" (default), "quaternion" or "none"', metavar="METHOD")

    # Reorder arguments
    parser.add_argument('-e', '--reorder', action='store_true',
                        help='align the atoms of molecules (default: Hungarian)')
    parser.add_argument('--reorder-method', action='store', default="hungarian", metavar="METHOD",
                        help='select which reorder method to use; hungarian (default), brute, distance')
    parser.add_argument('--use-reflections', action='store_true',
    ...
    parser.add_argument('--use-reflections-keep-stereo', action='store_true',
    ...
    index_group.add_argument('--remove-idx', nargs='+', type=int, help='index list of atoms NOT to consider',
                             metavar='IDX')
    index_group.add_argument('--add-idx', nargs='+', type=int, help='index list of atoms to consider', metavar='IDX')
    index_group.add_argument('--select', action='store_true',
                             help='Select atoms in FIRST pdb by chain, residue number and atom type')

    # Additional options for select
    parser.add_argument('--chain', type=str, help='chain name, e.g. \'A\'')
    parser.add_argument('--res-range-a', type=str, help='Range of residue numbers to considure, e.g. 1-10')
    parser.add_argument('--res-range-b', type=str, help='Range of residue numbers to considure, e.g. 1-10')
    parser.add_argument('--atom-names', type=str, help='Atom names, comma-separated list, e.g. N,CA,C,O')

    # format and print
    parser.add_argument('--format', action='store', help='format of input files. valid format are xyz and pdb',
                        metavar='FMT')
    parser.add_argument('-p', '--output', '--print', action='store_true',
                        help='print out structure B, centered and rotated unto structure A\'s coordinates in XYZ format')

    parser.add_argument('--max-rmsd', type=float, help='Maximum RMSD to print, ingore otherwise', default=1e100)
    ...
    if not args.select and (args.chain or args.res_range_a or args.res_range_b or args.atom_names):
        msg = "error: --chain, --res-range_a(b) and --atom-names should be used with --select option only"
        eprint(msg)
        exit()

    res_range_list_a = list()
    res_range_list_b = list()
    res_range_name_a = "ALL"
    res_range_name_b = "ALL"
    atom_names_set = set()
    if args.select:
        if args.res_range_a:
            try:
                res_start_stop = args.res_range_a.split('-')
                if len(res_start_stop) >= 2:
                    res_range_list_a = range(int(res_start_stop[0]), int(res_start_stop[1]) + 1)
                    res_range_name_a = "{:4} - {:4}".format(int(res_start_stop[0]), int(res_start_stop[1]))
            except:
                msg = "error: could not parse --res-range-a {}".format(args.res_range_a)
                eprint(msg)
                exit()
        if args.res_range_b:
            try:
                res_start_stop = args.res_range_b.split('-')
                if len(res_start_stop) >= 2:
                    res_range_list_b = range(int(res_start_stop[0]), int(res_start_stop[1]) + 1)
                    res_range_name_b = "{:4} - {:4}".format(int(res_start_stop[0]), int(res_start_stop[1]))
            except:
                msg = "error: could not parse --res-range-b {}".format(args.res_range_b)
                eprint(msg)
                exit()

        if args.atom_names:
            atom_names_set = set(args.atom_names.split(','))
            # aprint = ','.join(list(atom_names_set))
            # print("Atom types {} will be read fron file {}".
            #      format(aprint, args.structure_a))
    ...
    if args.format is None:
        ...
        if filename_suffix != "gz":
            ...
            if filename_suffix == 'pdb':
                pdb_flag = True
        elif len(filename_split) >= 3:
            filename_suffix2 = filename_split[-2]
            args.format = filename_suffix2 + "." + filename_suffix
            if filename_suffix2 == 'pdb':
                pdb_flag = True
        else:
            args.format = filename_suffix

    if args.select and pdb_flag:
        p_all_atoms, p_all = get_coordinates_pdb(args.structure_a,
                                                 chain=args.chain, res_range=res_range_list_a,
                                                 atom_names=atom_names_set)
        q_all_atoms, q_all = get_coordinates_pdb(args.structure_b,
                                                 chain=args.chain, res_range=res_range_list_b,
                                                 atom_names=atom_names_set)
    else:
    ...
    if not p_size == q_size:
        eprint("error: Structures not same size:")
        eprint("{} atoms read from {}".format(p_size, args.structure_a))
        eprint("{} atoms read from {}".format(q_size, args.structure_b))
        quit()
...
"""
    ...
    if p_view is None:
        ...
        if args.reorder and args.output:
            eprint("error: Cannot reorder atoms and print structure, when excluding atoms (such as --no-hydrogen)")
            quit()

        if args.use_reflections and args.output:
            eprint("error: Cannot use reflections on atoms and print, when excluding atoms (such as --no-hydrogen)")
            quit()
    ...
    if args.rotation.lower() == "kabsch":
        ...
        eprint("error: Unknown rotation method:", args.rotation)
        quit()
    ...
    if args.reorder_method == "hungarian":
        ...
        eprint("error: Unknown reorder method:", args.reorder_method)
        quit()
    ...
    if args.use_reflections:
        ...
        if not all(p_atoms == q_atoms):
            eprint("error: Structure not aligned")
            quit()
    ...
    if args.output:
        ...
        if args.reorder:
            ...
            if q_review.shape[0] != q_all.shape[0]:
                eprint("error: Reorder length error. Full atom list needed for --print")
                quit()
        ...
        if result_rmsd < args.max_rmsd:
            print("{0:>20} {1:<11} {2:>20} {3:<11}    {4:>6.4f}".format(args.structure_a, res_range_name_a,
                                                                        args.structure_b, res_range_name_b,
                                                                        result_rmsd))
